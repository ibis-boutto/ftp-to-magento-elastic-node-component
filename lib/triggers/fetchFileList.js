...
async function attachFile(source, filePath, cfg) {
  ...
  source.logger.info('Trying to attach ' + filePath);
  let uploadResult;
  try {
    uploadResult = await attachmentProcessor.uploadAttachment(fileStream);
    source.logger.info('File was attached ' + filePath);
  } catch (error) {
    source.logger.info('Error Attaching ' + filePath + " - ERROR: " + error);

  }
  const attachmentUrl = uploadResult.config.url;
  source.logger.info('File is successfully uploaded as attachment to URL ' + attachmentUrl);
