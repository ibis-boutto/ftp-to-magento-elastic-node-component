...
function attachFile(file) {
  return new Promise((resolve, reject) => {
    ftpClient.get(fullPath, async (err, stream) => {
      if (err) reject("Error Processing file " + file + ". Message: " + err);
      try {
        uploadResult = await attachmentProcessor.uploadAttachment(stream);
...
exports.process = async function fetchFileList(msg, cfg) {
  ...
  fileList.forEach(async (file) => {
    ...
    try {
      attachmentUrl = await attachFile(path.join(cfg.path, file.name));
    } catch (error) {
