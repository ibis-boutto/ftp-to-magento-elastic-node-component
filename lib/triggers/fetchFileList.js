...
exports.process = async function fetchFileList(msg, cfg, snapshot) {
  ...
  const polledFiles = fileList.filter(file => {
    return !fileKeyExists(file, newSnapShot) || !newSnapShot.keys ||
      newSnapShot.keys.length === 0;
  }).length

  this.logger.info('Snapshot contains ', newSnapShot.keys.length + " items and " +
    polledFiles + " will be added.");
